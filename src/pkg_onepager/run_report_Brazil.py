...
def run_report_Brazil():
    ...
    ##################################################################
    receiver_temp = config.email_receiver
    config.email_receiver = ["rubensh@iadb.org"]
    #config.email_receiver = ["markets.reporting@gmail.com"]    
    ...
    ##################################################################
    config.email_subject = '[SRM Analytics] Markets Monitor | Brazil | ' + ut.format_date(config.today_dt)
    ...
    config.email_receiver = receiver_temp
